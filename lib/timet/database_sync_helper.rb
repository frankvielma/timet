...
module Timet
  ...
  module DatabaseSyncHelper
    ...
    def self.sync_databases(local_db, remote_db, s3, bucket, local_db_path)
      ...
      # Convert items to hash for easier comparison
      remote_by_id = remote_items.to_h { |item| [item['id'], item] }
      local_by_id = local_items.to_h { |item| [item['id'], item] }
      ...
      all_ids.each do |id|
        ...
        if remote_item && local_item
          ...
          if local_time > remote_time
            ...
            puts "Item #{id} has same timestamp - no update needed"
          end
        elsif remote_item
      ...
      # Upload the merged database back to S3
      s3.upload_file(bucket, local_db_path, 'timet.db')
